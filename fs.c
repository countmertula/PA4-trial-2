...
dirlink(struct inode *dp, char *name, uint inum)
{
  ...
  //check if directory is a smalldirectory and if so make sure the file is a small file
  if(dp->type == T_SMALLDIR){
  //fairly simple checks if the file isn't too big for the directory
  //before allowing it to be inserted
  if(inum != T_SMALLFILE && !namecmp(name, ".") && !namecmp(name, "..")){
    return -1;
  }
}
